...
% need a bit more stuff here.. one or two examples
...
The effects of these conditions on the ground are most often called the \textit{impact} of a certain weather phenomenon or climate development such as a hazard. The level of impact is commonly determined based on the severity of the hazard and on the prevailing \textit{risk} level on site TODO: source (?). 

The special report on drought defines Risk as ..  with vulnerability... 

plus --> https://www.droughtmanagement.info/literature/UNCCD_FAO_GWP_WMO_2019_Drought_Vulnerability.pdf


this is nearly identical (??) to the definition of the IFRC 

Impact can furthermore be .... % what ever else is in the source
...
All of these drought indices give a good impression about the physical side of climate anomalies, but none of the aboce mentioned indices link those climate anomalies to socioeconomic vulnerabilities \autocite{enenkelWhyPredictClimate2020}. The simmple but widely used Falkenmark Indicator (Falkenmark et al. 1989) incorporates human factors by calculating a ration between the given amount of water and the number of people living within that domain. By further categorizing this ratio to a level of water scarcity, the Falkenmark Indicator indicates the supply sides effects of water scarcity. Variabilities, demand and socioeconomic factors are not represented. 


More dedicated indices like the \acrfull*{iwmi} Indicator and the \acrfull*{wpi} Water Poverty Index (WPI) as well as other indices measuring water security give a more extensive representation of the overall situation \autocite{arreguin-cortesMunicipalLevelWater2019,liuWaterScarcityAssessments2017}. The \arcshort{wpi} for example represents the weighted average of five pre-standardized components namely, water availability, access, capacity, use and environment. 

% here. contirnue with the above stuff..
...
and while socio-economic indicators ...

\autocite{enenkelWhyPredictClimate2020} has highlighted


TODO: look if this worked. 
...
this wide range of potential indicators and indices can also be found for water scarcity and water security. (is that correct??). For water scarcity the xy xy xy indices are prominent and for water security xy xy xy.



continue with the already written stuff about determining etc.
