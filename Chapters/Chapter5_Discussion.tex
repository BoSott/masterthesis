...
% lit discussion Kernpunkte
- lit review could underline the assumption, that no other framework exists that fit
...
- Especially not, with the focus on providing feasible information for triggering \acrshortpl{aa} in the context of an \acrshort{eap} and in collaboration with a \acrshort{rcrc} National Society.
...
% somewhat expected but this was really not a lot
...
% further gaps in drought FbF - partly unexpected
...
Thus, each new project or study focussing on this hazard in the context of \acrshort{fbf} has, at least in part, an exploratory character.\newline
...
% ! framework
% all models are wrong, but some are useful. --> other frameworks group stuff differently, same as with research methods: advantages and disadvantages... just be concious about it.

% not always easy to clearly put one thing into one stage -> interconnectedness and so on..
% e.g. networks
% not detrimental, just a certain perspective
% counter acted by the PRC

% auch andere anderen von frameworks finden sich hier wieder wie... 
...
evaluation criteria: https://discovery.ucl.ac.uk/id/eprint/10066007/1/Chapter%206.pdf
...
und nicht mit den goals: in der Art der erste und einzige: ggf. aus gutem Grund? Ich fand's jedenfalls sinnvoll..
...
%! text design roadmap Klappe die Erste
